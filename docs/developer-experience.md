...
| Game repository | The form-free 0.1.5 controls activated through mouse, touch, and keyboard, but the first player-state write was rejected against a high revision left by the previously selected built-in game. The runtime adapter started before that production revision was echoed, and an event-only mutation path returned accepted without a usable UI error. | On an explicit `stale-revision` response, retry the same controller-owned intent exactly once at the server-confirmed revision. Keep the retry inside the runtime adapter, cover it with a failing-first focused test, and continue rendering only confirmed subscription echoes. Acceptance is a one-activation production submission for each isolated controller. |

These compatibility and packaging changes are game-specific and were fixed in the standalone repository. The broader risks—local Workbench roles differ from production roles, a newly selected game can receive the prior game's canonical snapshot, and the scaffold's copied HTML does not automatically link Vite's extracted CSS—were deduplicated and filed as reusable platform DX feedback.
...
Version `0.2.1` was published publicly from commit `fe71d83`. Fresh room `05M6FQ` loaded the exact versioned host and controller assets, assigned room authority to exactly one controller, accepted three covers and three ballots from isolated mobile/touch contexts, committed results and personal feedback, and reported zero interactive or focusable host elements. The checked-in production screenshot confirms the generated logo and distressed comic stage render from the registry alongside the lead-answer wall and score race. No additional production-only defects remained.
