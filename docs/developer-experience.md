...
These compatibility changes are game-specific and were fixed in the standalone repository. The broader risks—local Workbench roles differ from production roles, and a newly selected game can receive the prior game's canonical snapshot—will be deduplicated against platform issues before filing reusable DX feedback.
