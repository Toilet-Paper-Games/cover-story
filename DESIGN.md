...
Cover Story looks like an unofficial late-1990s school yearbook documenting events that could not have happened. Cool paper, editorial grids, bold Georgia headlines, cobalt school ink, correction-red annotations, trophy-yellow seals, slightly crooked artifacts, and portrait collage artwork make every phase feel like another spread in the same permanent record.
...
- **Correction Red** (`#bd2028`): eyebrows, motives at reveal, countdowns, ranks, errors, and editorial annotation.
...
**Body Font:** Inter when available, then native system sans-serif fallbacks
...
Controllers are centered personal pages up to `720px` wide with viewport-safe padding. Writing keeps the incident, motive, input, character count, deadline, and submit action in one vertical flow. Voting is intentionally split into Decode and Crown steps to reduce phone-length cognitive load while preserving selections across confirmed runtime echoes. For viewports at most `380px × 650px`, headings, margins, quote cards, choices, and actions compact without reducing the touch target below `50px`.
...
- **Answer stage:** up to four white answer cards per automatic page; cover text, correction-red motive, author/vote/decode byline, and page position remain visible together.
...
- **Choices:** native radio inputs inside full-row labels with ink dividers; minimum action height remains phone-friendly.
