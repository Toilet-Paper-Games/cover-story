...
export class ControllerSurfaceRenderer {
  ...
  private phaseBody(view: ControllerViewModel): string {
    if (view.phase === "connecting") return panel("Tuning into the studio…", "Waiting for the room's confirmed state.");
    ...
    if (view.phase === "lobby") return panel("You’re in the cover-up crew", `${view.players.length} joined · the first case opens with 3 players.${view.isAuthority ? " You are the room director; start the game from the TP Games room controls." : ""}`);
    if (view.phase === "instructions") {
      ...
      return `<section class="controller-panel"><p class="phase-ribbon">Three quick moves</p><h1>Cover. Decode. Crown.</h1><ol class="phone-rules"><li><b>Cover</b> the incident in one sentence.</li><li><b>Hide</b> your exact private motive.</li><li><b>Expose</b> a rival, then crown a favorite.</li></ol>${countdown(view.countdownAt)}<button class="primary-action" data-action="ack" ${view.writePending ? "disabled" : ""}>${view.writePending ? "Locking in…" : "Enter the studio"}</button></section>`;
    }
    if (view.phase === "round-intro") return `<section class="controller-panel"><p class="phase-ribbon">${html(view.roundLabel)} · New case</p><h1>${html(view.incident ?? "New incident")}</h1>${motiveCard(view)}<p class="helper">Hint at this motive. Never type its exact words.</p>${countdown(view.countdownAt)}</section>`;
    if (view.phase === "writing") {
      ...
      return `<section class="controller-panel"><p class="phase-ribbon">${html(view.roundLabel)} · Build your alibi</p><h1>${html(view.incident ?? "Explain yourself")}</h1>${motiveCard(view)}<div data-form="cover"><label for="cover">Your one-sentence cover</label><textarea id="cover" name="cover" minlength="3" maxlength="140" rows="4" required placeholder="Obviously, this happened because…">${html(this.draft)}</textarea><div class="form-meta"><span data-count>${this.draft.length}/140</span>${countdown(view.countdownAt)}</div><div class="action-dock"><button class="primary-action" type="button" data-action="submit-cover" ${view.writePending ? "disabled" : ""}>${view.writePending ? "Locking in…" : "Lock in my cover"}</button></div></div></section>`;
    }
    if (view.phase === "voting") {
      ...
      if (this.ballotStep === "decode") {
        return `<section class="controller-panel"><div class="ballot-steps" aria-label="Voting step 1 of 2"><b>1 Decode</b><span>2 Crown</span></div><p class="phase-ribbon">Expose the motive</p><h1>What drove this alibi?</h1><div class="quote-card"><span aria-hidden="true">“</span><p>${html(target?.text ?? "Cover unavailable")}</p></div>${countdown(view.countdownAt)}<div data-form="decode"><fieldset><legend>Choose the hidden motive</legend>${view.ballot.angleOptions.map((angle) => radio("angleGuessId", angle.id, angle.label, angle.id === this.ballotAngleGuessId)).join("")}</fieldset><div class="action-dock"><button class="primary-action" type="button" data-action="continue-decode">Continue to favorite</button></div></div></section>`;
      }
      return `<section class="controller-panel"><div class="ballot-steps" aria-label="Voting step 2 of 2"><span>✓ Decode</span><b>2 Crown</b></div><p class="phase-ribbon">Choose the headline</p><h1>Which alibi wins?</h1>${countdown(view.countdownAt)}<div data-form="ballot"><fieldset><legend>Choose your favorite cover</legend>${view.ballot.favoriteAnswerIds.map((id) => radio("favoriteAnswerId", id, Object.values(view.submissions).find((answer) => answer.id === id)?.text ?? "Cover unavailable", id === this.ballotFavoriteAnswerId)).join("")}</fieldset><div class="action-dock"><button class="primary-action" type="button" data-action="submit-ballot" ${view.writePending ? "disabled" : ""}>${view.writePending ? "Submitting…" : "Submit my ballot"}</button></div><button class="secondary-action" type="button" data-action="back-decode">Change my decode</button></div></section>`;
    }
    if (view.phase === "results") {
      ...
      const decode = view.personalResult
        ...
        : "";
      return `<section class="controller-panel"><p class="phase-ribbon">Case score</p><h1>${score ? `${score.score} points` : "Results are in"}</h1>${decode}<p class="deck">Favorites are worth 100. Correct decodes are worth 60. Being decoded is worth 40.</p>${scoreRows(view.scoreboard, true)}${countdown(view.countdownAt)}</section>`;
    }
    if (view.phase === "finale") return `<section class="controller-panel controller-finale"><p class="phase-ribbon">Final headline</p><h1>${view.scoreboard[0]?.id === view.playerId ? "Cover Legend" : "The final score"}</h1>${scoreRows(view.scoreboard)}${view.isAuthority ? '<button class="primary-action" data-action="lobby">Return everyone to lobby</button><button class="secondary-action" data-action="settings">Room settings</button>' : '<p class="helper">The room director controls what happens next.</p>'}</section>`;
...
function panel(title: string, copy: string, detail = ""): string {
  return `<section class="controller-panel"><p class="phase-ribbon">Live studio</p><h1>${html(title)}</h1><p class="deck">${html(copy)}</p>${detail}</section>`;
}
...
function submitted(title: string, copy: string, view: ControllerViewModel): string {
  return `<section class="controller-panel submitted-panel"><div class="check-seal"><span>✓</span></div><p class="phase-ribbon">Submitted</p><h1>${html(title)}</h1><p class="deck">${html(copy)}</p>${countdown(view.countdownAt)}</section>`;
}
